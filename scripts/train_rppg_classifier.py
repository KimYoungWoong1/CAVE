"""레이어 4 rPPG feature classifier 학습.

FFPP C23 real/deepfake 영상에서 CHROM rPPG feature를 뽑고,
RandomForest 기반 physiological anomaly detector를 학습한다.

실행:
  python scripts/train_rppg_classifier.py --max-per-label 30 --fake-per-method 6
"""
...
from layers.rppg_check import RPPG_CLASSIFIER_PATH  # noqa: E402
from layers.rppg_check import RPPG_FEATURE_ORDER  # noqa: E402
from layers.rppg_check import extract_rppg_features  # noqa: E402
from layers.rppg_check import rppg_feature_vector  # noqa: E402
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CAVE rPPG RF classifier 학습")
    ...
    parser.add_argument("--output", default=str(RPPG_CLASSIFIER_PATH))
    parser.add_argument("--max-per-label", type=int, default=30)
    parser.add_argument("--eval-max-per-label", type=int, default=24)
    parser.add_argument("--fake-per-method", type=int, default=6)
    parser.add_argument("--eval-fake-per-method", type=int, default=4)
    ...
    raw_eval_prob = model.predict_proba(x_eval)[:, _fake_index(model)]
    raw_auc = _auc(y_eval, raw_eval_prob)
    invert_probability = raw_auc < 0.5
    eval_prob = 1.0 - raw_eval_prob if invert_probability else raw_eval_prob
    metrics = _classification_metrics(y_eval, eval_prob)
    method_metrics = _method_metrics(eval_rows, eval_prob)

    meta = {
        "task": "rppg_feature_real_fake_anomaly_detection",
        "data_source": "FaceForensics++ C23",
        "feature_order": RPPG_FEATURE_ORDER,
        ...
        "note": "RandomForest classifier over CHROM rPPG physiological features.",
    }
    ...
    print(f"rPPG classifier saved: {output}")
...
def _collect_rows(
    ...
    for path in real_files:
        row = _analyze_path(path, label=0, method="real")
        if row is None:
            skipped += 1
        else:
            rows.append(row)
    ...
    for path in fake_files:
        row = _analyze_path(path, label=1, method=_method_from_file(path))
        if row is None:
            skipped += 1
        else:
            rows.append(row)
...
def _analyze_path(path: Path, label: int, method: str) -> dict | None:
    try:
    ...
    return {
        ...
        "method": method,
        "features": features,
        "vector": rppg_feature_vector(features),
...
def _method_metrics(rows: list[dict], scores: np.ndarray) -> dict[str, dict]:
    ...
    for method in methods:
        ...
        values = np.asarray(real_scores + method_scores, dtype=float)
        result[method] = {
            "count": len(method_scores),
            "prob_mean": float(np.mean(method_scores)) if method_scores else 0.0,
            "auc_vs_real": _auc(labels, values) if real_scores and method_scores else 0.5,
        }
    return result
