...
def render_pipeline() -> Image.Image:
    ...
    x = 800
    input_box = (430, 50, 1170, 180)
    l1_box = (420, 250, 765, 380)
    l2_box = (835, 250, 1180, 380)
    l3_box = (430, 530, 1170, 660)
    l4_box = (430, 730, 1170, 860)
    l5_box = (430, 930, 1170, 1060)
    l6_box = (430, 1130, 1170, 1260)
    authentic_box = (115, 1380, 465, 1505)
    clash_box = (600, 1380, 1000, 1505)
    suspected_box = (1135, 1380, 1485, 1505)
    l7_box = (430, 1685, 1170, 1815)
    output_box = (430, 1900, 1170, 2030)
    metrics_box = (110, 2090, 1490, 2325)

    pipeline_box(draw, input_box, "디지털 파일 입력", "이미지 · 영상", fill="#f5f2ea", outline="#e5e7eb")
    down_arrow(draw, x, 180, 250)

    layer_label(draw, 86, 310, "출처\n레이어")
    dashed_guide(draw, 190, 315, 405, 315)
    ...
    center_text(draw, 592, 405, "암호화 표준 (비AI)", font(21), "#a3e635")
    center_text(draw, 1008, 405, "신호처리 (비AI)", font(21), "#a3e635")
    connect_to_center(draw, l1_box, x, 480)
    connect_to_center(draw, l2_box, x, 480)
    down_arrow(draw, x, 480, 530)

    layer_label(draw, 86, 595, "AI\n탐지")
    dashed_guide(draw, 190, 605, 405, 605)
    pipeline_box(draw, l3_box, "L3 · AI 탐지 모델", "sdxl-detector · EfficientNet · Xception · R3D 앙상블", fill="#ecebff", outline="#8b5cf6", title_color="#4338ca")
    down_arrow(draw, x, 660, 730)
    pipeline_box(draw, l4_box, "L4 · rPPG 생체 신호", "CHROM rPPG + RandomForest · 영상 전용 보조 신호", fill="#ecebff", outline="#8b5cf6", title_color="#4338ca")
    down_arrow(draw, x, 860, 930)
    pipeline_box(draw, l5_box, "L5 · 생성 모델 핑거프린트", "FFT · 잔차 노이즈 · RF attribution (RedFace)", fill="#ecebff", outline="#8b5cf6", title_color="#4338ca")
    down_arrow(draw, x, 1060, 1130)

    layer_label(draw, 86, 1198, "통합\n판정")
    dashed_guide(draw, 190, 1200, 405, 1200)
    ...
    down_arrow(draw, x, 1260, 1322)
    draw.line((280, 1322, 1320, 1322), fill="#9ca3af", width=2)
    branch_arrow(draw, 280, 1322, center_top(authentic_box))
    branch_arrow(draw, x, 1322, center_top(clash_box))
    branch_arrow(draw, 1320, 1322, center_top(suspected_box))
    ...
    pipeline_box(draw, suspected_box, "AI 생성 의심", "생성 가능성 높음", fill="#fee2e2", outline="#fb7185", title_color="#7f1d1d", title_size=28, body_size=22)
    draw.line((280, 1630, 1320, 1630), fill="#9ca3af", width=2)
    draw.line((280, 1505, 280, 1630), fill="#9ca3af", width=2)
    draw.line((800, 1505, 800, 1630), fill="#9ca3af", width=2)
    draw.line((1320, 1505, 1320, 1630), fill="#9ca3af", width=2)
    down_arrow(draw, x, 1630, 1685)

    layer_label(draw, 86, 1740, "피해\n산정")
    dashed_guide(draw, 190, 1745, 405, 1745)
    pipeline_box(draw, l7_box, "L7 · 피해 규모 정량화", "GNN 확산 추적 · 재유포 RF · 가중 합산 30점", fill="#ecebff", outline="#8b5cf6", title_color="#4338ca")
    down_arrow(draw, x, 1815, 1900)

    layer_label(draw, 86, 1955, "출력")
    dashed_guide(draw, 190, 1965, 405, 1965)
    pipeline_box(draw, output_box, "L8 · 웹 결과 요약 / 기술 분석 리포트", "판정 근거 · 피해 점수 · 정밀 감정 권고", fill="#f5f2ea", outline="#e5e7eb")

    draw.rounded_rectangle(metrics_box, radius=18, fill="#050607", outline="#e5e7eb", width=2)
    center_text(draw, 800, 2115, "로컬 모델 평가 결과 (RedFace · GenImage · FF++ C23)", font(20), "#a3e635")
    metric_text(draw, 290, 2160, "L5 핑거프린트 (RedFace)", metrics["l5_auc"])
    metric_text(draw, 560, 2160, "L3 GenImage 분류기", metrics["l3_auc"])
    metric_text(draw, 860, 2160, "L4 rPPG (FF++ C23)", metrics["l4_auc"])
    metric_text(draw, 1160, 2160, "L5 attribution", metrics["attribution"])
    center_text(draw, 800, 2265, "L4 rPPG는 보조 신호로만 활용 / 법적 확정 판정기 아님", font(20), "#a3e635")

    return img


def read_pipeline_metrics() -> dict[str, str]:
    general_aigc_meta = read_json("general_aigc_classifier.meta.json")
    fingerprint_meta = read_json("fingerprint_classifier.meta.json")
    rppg_meta = read_json("rppg_classifier.meta.json")
    return {
        "l3_auc": f"AUC {general_aigc_meta.get('test_auc', 0.0):.3f}",
        "l5_auc": f"AUC {fingerprint_meta.get('test_auc', 0.0):.3f}",
        "l4_auc": f"AUC {rppg_meta.get('test_auc', 0.0):.3f}",
        "attribution": f"Acc {fingerprint_meta.get('method_accuracy', 0.0) * 100:.1f}%",
    }
