"""GenImage 기반 Layer 5 generator fingerprint classifier 학습."""
...
from layers.fingerprint import GENIMAGE_FINGERPRINT_CLASSIFIER_PATH  # noqa: E402
...
def main() -> None:
    parser = argparse.ArgumentParser(description="GenImage generator fingerprint classifier 학습")
    ...
    parser.add_argument("--output", default=str(GENIMAGE_FINGERPRINT_CLASSIFIER_PATH))
    ...
    if len(train_rows) < 10 or len(eval_rows) < 4:
        raise SystemExit("GenImage fingerprint 학습/평가 샘플이 부족합니다.")
    ...
    generator_model = _train_generator_model([row for row in train_rows if row["label"] == 1], args.seed)

    eval_prob = binary_model.predict_proba(x_eval)[:, _ai_index(binary_model)]
    metrics = _classification_metrics(y_eval, eval_prob)
    generator_accuracy = _generator_accuracy(generator_model, eval_rows)
    generator_metrics = _generator_metrics(eval_rows, eval_prob)

    meta = {
        "task": "genimage_fingerprint_real_ai_and_generator_attribution",
        ...
        "note": "RandomForest classifier over FFT/residual/noise features for GenImage generator attribution.",
    }
    bundle = {
    ...
    print(
        ...
        f"generator_acc={generator_accuracy:.3f}"
    )
...
def _train_generator_model(fake_rows: list[dict], seed: int):
    from sklearn.ensemble import RandomForestClassifier

    x_train = np.asarray([row["vector"] for row in fake_rows], dtype=np.float64)
    y_train = np.asarray([row["generator"] for row in fake_rows], dtype=object)
    model = RandomForestClassifier(
        n_estimators=350,
        max_depth=14,
        min_samples_leaf=2,
...
def _collect_rows(real_dir: Path, ai_dir: Path, max_per_label: int, seed: int) -> list[dict]:
    ...
    for path in real_files:
        rows.append(_analyze_path(path, label=0, generator="real"))
...
def _generator_accuracy(model, rows: list[dict]) -> float:
    fake_rows = [row for row in rows if row["label"] == 1]
    if not fake_rows:
        return 0.0
    vectors = np.asarray([row["vector"] for row in fake_rows], dtype=np.float64)
    labels = np.asarray([row["generator"] for row in fake_rows], dtype=object)
